...
public class PrivateForm {
    ...
    public static String createPrivateFormPayload() {
    JSONObject payload = new JSONObject();
    String formName = FormNameGenerator.getNextFormName();

    payload.put("name", formName);
    payload.put("schedule_type", "MONTHLY"); // Changed
    payload.put("is_required_form_serial_number", true);
    payload.put("form_type", "PRIVATE"); // Changed
    payload.put("auto_work_order_assignment", true);
    payload.put("reporting", true); // Changed
    payload.put("digital_signature", true); // Changed
    ...
    components.put(new JSONObject().put("id", 24).put("root_id", JSONObject.NULL)); // Root
    components.put(new JSONObject().put("id", 30).put("root_id", 24));              // Child
    payload.put("components", components);

    // Users and Teams
    payload.put("users", new JSONArray().put(153));
    payload.put("teams", new JSONArray());
    payload.put("reporting_users", new JSONArray().put(153));
    payload.put("reporting_teams", new JSONArray());

    // Schedule Setting: dynamically generated
    ...
    ZonedDateTime endDate = now.plusDays(30);
